...
function stories(endings, choices, choice) {
  if ((!endings, !choices)) return -1;
  ...
  for (const arr of choices) {
    lookup[arr[0]] = arr[choice];
  }

  while (page < endings[endings.length - 1] || visitCounter <= 2) {
    if (visitCounter > 2) return -1;
    ...
    if (page === endings[page]) return endings[page];
    page++;
  }
...
console.log(stories(endings2, choices2_1, 1));
